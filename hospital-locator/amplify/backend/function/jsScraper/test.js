...
var scrapeRatesInfo = (tr) => {
    ...
    var text = cheerio.load(packageButton[0]).text() // load into cheerio to acquire nested text
...
(async () => {
    ...
    fs.readFile('scrape.txt', function (err, data) {
        if (err) return console.log(err);
        
        var features = []
        var $ = cheerio.load(data)
        
        var trs = $('.wrapper-tbl .hospital-status tbody tr')
        trs.each((i, el) => {
            try {
                var tr = cheerio.load(el)
                if (isValidTr(tr)) {
                    var feature = {
                        "type": "Feature",
